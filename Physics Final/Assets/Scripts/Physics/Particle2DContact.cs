...
public class Particle2DContact : MonoBehaviour
{
	...
	public Particle2DContact(Particle2D obj1, Particle2D obj2, double restitution, Vector2 normal, double penetration, Vector2 move1, Vector2 move2)
	{
	...
	void resolveVelocity(double dt)
	{
		...
		double totalInverseMass = mObj1.getInverseMass();
		if (mObj2)
		...
		float impulse = deltaVel / (float)totalInverseMass;
		Vector2 impulsePerIMass = mContactNormal * impulse;

		Vector2 newVelocity = mObj1.mVel + impulsePerIMass * (float)mObj1.getInverseMass();
		mObj1.mVel = newVelocity;
		if (mObj2)
		{
			newVelocity = mObj2.mVel + impulsePerIMass * -(float)mObj2.getInverseMass();
	...
	void resolveInterpenetration(double dt)
	{
		...
		float totalInverseMass = (float)mObj1.getInverseMass();
		if (mObj2)
			totalInverseMass += (float)mObj2.getInverseMass();
		...
		mMove1 = movePerIMass * (float)mObj1.getInverseMass();
		if (mObj2)
			mMove2 = movePerIMass * (float)-mObj2.getInverseMass();
		else
			mMove2 = Vector2.zero;
